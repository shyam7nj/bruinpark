...
// Test: Log in as a different user -> Browse posts -> Send a message request to Jiahao's "Club meet" post
test("test browse user can send a message request to Jiahao's Club meet post", async ({ page }) => {
    const CLUB_MEET_POST_ID = "6a213a55219e28b11e699b54";
    const SENDER_EMAIL = "test-message-sender@g.ucla.edu";

    // Clean up any leftover pending request from a previous run so the test is repeatable
    await page.request.delete(`${BACKEND_URL}/auth/cleanup-message-request`, {
        data: { postId: CLUB_MEET_POST_ID, senderEmail: SENDER_EMAIL },
    });

    // Log in as a separate test user so we are not messaging ourselves
    await testLogin(page, {
        email: SENDER_EMAIL,
    ...
    // Filter to Structure 7 so the post list is narrow and Club meet is easy to find
    await page.getByLabel(/parking structure/i).selectOption("Structure 7");

    // Wait for at least one post card to appear, then locate the Club meet post
    await expect(page.locator(".browse-post-card").first()).toBeVisible();
    const clubMeetPost = page.locator(".browse-post-card").filter({ hasText: "Club meet" });
    await expect(clubMeetPost).toBeVisible();
    await expect(clubMeetPost).toContainText("Structure 7");

    // Open the message request form
    await clubMeetPost.getByRole("button", { name: /send message request/i }).click();

    // Fill in a message and send it
    await clubMeetPost.getByLabel(/message request/i).fill("Hey! I saw your Club meet post — interested in swapping permits.");
    await clubMeetPost.getByRole("button", { name: /send request/i }).click();

    // Confirm the request was sent successfully
    await expect(clubMeetPost.getByText(/message request sent/i)).toBeVisible();
});
