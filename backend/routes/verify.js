...
router.post('/check', requireAuth, async (req, res) => {
  ...
  try {
    const result = await checkInboxForToken(req.user.email, token);
    ...
    return res.status(400).json({
      isVerified: false,
      message: result.reason || 'Permit email not found or invalid. Please check your forwarded email and try again.',
    });
  } catch (err) {
...
router.post('/admin-bypass', requireAuth, async (req, res) => {
  ...
  console.log('Admin bypass attempt, password received:', JSON.stringify(password));

  if (!password || password.trim() !== 'bruinpark') {
